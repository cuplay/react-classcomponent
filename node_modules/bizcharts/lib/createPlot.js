...
var BasePlot = /** @class */ (function (_super) {
    ...
    BasePlot.prototype.componentDidMount = function () {
        ...
        polyfill_1.polyfillEvents(this.g2Instance, {}, this.props);
        this.g2Instance.data = this.props.data;
        this.preConfig = pickWithout_1.default(this.props, __spreadArrays(constant_1.REACT_PIVATE_PROPS, [
            'container',
            'PlotClass',
            'onGetG2Instance',
            'data',
    ...
    BasePlot.prototype.componentDidUpdate = function (prevProps) {
        ...
        // 兼容1.0 的events写法
        polyfill_1.polyfillEvents(this.g2Instance, prevProps, this.props);
    };
    ...
    BasePlot.prototype.checkInstanceReady = function () {
        if (!this.g2Instance) {
            ...
            var options = pickWithout_1.default(this.props, [
                'container',
                'PlotClass',
                'onGetG2Instance',
                'children',
        ...
        // 缓存配置
        var currentConfig = pickWithout_1.default(this.props, __spreadArrays(constant_1.REACT_PIVATE_PROPS, [
            'container',
            'PlotClass',
            'onGetG2Instance',
            'data',
        ]));
        this.preConfig = cloneDeep_1.default(currentConfig);
        this.g2Instance.data = this.props.data;
    };
    BasePlot.prototype.initInstance = function () {
        ...
        if (is_function_1.default(onGetG2Instance)) {
    ...
    BasePlot.prototype.diffConfig = function () {
        ...
        return !shallowEqual_1.default(preConfig, currentConfig);
    };
    BasePlot.prototype.diffData = function () {
        // 只有数据更新就不重绘，其他全部直接重新创建实例。
        var preData = this.g2Instance.data;
        var data = this.props.data;
        if (!util_1.isArray(preData) || !util_1.isArray(data)) {
            // 非数组直接对比
            return !preData === data;
        }
    ...
    BasePlot.prototype.render = function () {
        ...
        return (react_1.default.createElement(root_1.default.Provider, { value: this._context },
            react_1.default.createElement(view_1.default.Provider, { value: chartView },
                react_1.default.createElement("div", { key: unique_id_1.default('plot-chart') }, this.props.children))));
    };
    return BasePlot;
}(react_1.default.Component));
var BxPlot = withContainer_1.default(BasePlot);
function createPlot(PlotClass, name, transCfg) {
    ...
    var Com = react_1.default.forwardRef(function (props, ref) {
        var title = props.title, description = props.description, _a = props.autoFit, autoFit = _a === void 0 ? true : _a, forceFit = props.forceFit, _b = props.errorContent, errorContent = _b === void 0 ? ErrorBoundary_1.ErrorFallback : _b, containerStyle = props.containerStyle, placeholder = props.placeholder, ErrorBoundaryProps = props.ErrorBoundaryProps, cfg = __rest(props, ["title", "description", "autoFit", "forceFit", "errorContent", "containerStyle", "placeholder", "ErrorBoundaryProps"]);
        var realCfg = transCfg(cfg);
        var container = react_1.useRef();
        var titleDom = react_1.useRef();
        var descDom = react_1.useRef();
        var _c = react_1.useState(0), chartHeight = _c[0], setChartHeight = _c[1];
        var resizeObserver = react_1.useRef();
        var resizeFn = react_1.useCallback(function () {
            if (!container.current) {
                return;
            }
            var containerSize = getElementSize_1.default(container.current);
            var titleSize = titleDom.current ? getElementSize_1.default(titleDom.current) : { width: 0, height: 0 };
            var descSize = descDom.current ? getElementSize_1.default(descDom.current) : { width: 0, height: 0 };
        ...
        var resize = react_1.useCallback(util_1.debounce(resizeFn, 500), [resizeFn]);
        var FallbackComponent = react_1.default.isValidElement(errorContent) ? function () { return errorContent; } : errorContent;
        ...
        if (placeholder && !realCfg.data) {
            ...
            return react_1.default.createElement(ErrorBoundary_1.default, __assign({ FallbackComponent: FallbackComponent }, ErrorBoundaryProps),
                react_1.default.createElement("div", { style: { width: props.width || '100%', height: props.height || 400, textAlign: 'center', position: 'relative' } }, pl));
        }
        var titleCfg = plotTools_1.visibleHelper(title, false);
        var descriptionCfg = plotTools_1.visibleHelper(description, false);
        ...
        if (!util_1.isNil(forceFit)) {
            warning_1.default(false, '请使用autoFit替代forceFit');
        }
        ;
        react_1.useEffect(function () {
            if (!isAutoFit) {
                ...
                if (container.current) {
                    resizeFn();
                    resizeObserver.current = new resize_observer_polyfill_1.default(resize);
        ...
        return react_1.default.createElement(ErrorBoundary_1.default, __assign({ FallbackComponent: FallbackComponent }, ErrorBoundaryProps),
            react_1.default.createElement("div", { ref: container, className: "bizcharts-plot", style: { position: 'relative', height: props.height || '100%', width: props.width || '100%' } },
                titleCfg.visible && react_1.default.createElement("div", __assign({ ref: titleDom }, polyfill_1.polyfillTitleEvent(realCfg), { className: "bizcharts-plot-title", style: titleStyle }), titleCfg.text),
                descriptionCfg.visible && react_1.default.createElement("div", __assign({ ref: descDom }, polyfill_1.polyfillDescriptionEvent(realCfg), { className: "bizcharts-plot-description", style: descStyle }), descriptionCfg.text),
                !!chartHeight && react_1.default.createElement(BxPlot
                // API 统一
                , __assign({ 
                    // API 统一
                    appendPadding: [10, 5, 10, 10], autoFit: isAutoFit, ref: ref }, realCfg, { PlotClass: PlotClass, containerStyle: __assign(__assign({}, containerStyle), { height: chartHeight }) }))));
    });
